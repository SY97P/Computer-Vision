...
{
	if (a.x - b.x < loosenPx && a.y - b.y < loosenPx)
		return true;
	return false;
}
...
int main()
{
	...
	first_image = cv::imread("florence-1.bmp", cv::IMREAD_GRAYSCALE);
	second_image = cv::imread("florence-2.bmp", cv::IMREAD_GRAYSCALE);
	...
	// keypoints_first.size = 48
	// keypoints_second.size = 24
	for (int key_i = 0; key_i < keypoints_first.size(); key_i++) {
		for (int key_j = 0; key_j < keypoints_second.size(); key_j++) {
			...
			// descriptor_vector_first.size = 7xx, descriptor_vectors_second.size = 3xx
			for (int desc_i = 0; desc_i < descriptor_vectors_first.size(); desc_i += 16)
				for (int desc_j = 0; desc_j < descriptor_vectors_second.size(); desc_j += 16)
				{
					...
					// Euclidean Distance = ((p1^2 - q1^2) + ... + (pn^2 - qn^2))^1/2
					// p = desriptor_vectors_first[key_i]

					double distance = 0.0;

					double temp = 0.0;
					for (int i = 0; i < 8; i++) {
						double p = descriptor_vectors_first[desc_i].descriptor[i];
						double q = descriptor_vectors_second[desc_j].descriptor[i];
						temp += pow(p - q, 2);
					}
					distance = sqrt(temp);
					/*if (distance < 0.09)
					...
					if (distance < 1.0) {
						...
						scale_first = pow(2, keypoints_first[key_i].octave);
						scale_second = pow(2, keypoints_second[key_j].octave);

						// printf("s_1 = %d\t", scale_first);
						// printf("s_2 = %d\n", scale_second);
	 ...
	 *
	 * 결과 이미지에 상위 3개의 원 쌍을 출력한다.
	 */

	 // Sort by distance (lower distance first!)
	...
	for (int i = 0; i < distance_pair.size() && i < 3; i++)
	{
		...
		cv::circle(first_image_output, cv::Point(point_set.firstImgPtr.x, point_set.firstImgPtr.y), 10, (0, 0, 255), 2, 8, 0);
		cv::circle(second_image_output, cv::Point(point_set.secondImgPtr.x, point_set.secondImgPtr.y), 10, (255, 0, 0), 2, 8, 0);
