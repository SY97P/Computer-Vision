...
EquationElement algorithms::LeastSquare::calculate_least_square()
{
	...
	// 기울기 a를 구하기 위해서 분모, 분자 파트를 나누어 summation
...
void CRANSAC::convert_two_point_to_line()
{
	...
	// ** 지금부터 코드를 작성하세요. 이 줄은 지우시면 안 됩니다 **
	lineEquation.m = (pt2.x - pt1.x) == 0 ? 0 : (pt2.y - pt1.y) / (pt2.x - pt1.x);
...
void CRANSAC::calculate_inlier()
{
	...
	for (int i = 0; i < point_vector_list.size(); i++)
	{
		if (i != pt1Idx && i != pt2Idx)
		{
			...
			// ** 지금부터 코드를 작성하세요. 이 줄은 지우시면 안 됩니다 **
			double up = (-1 * (point_vector_list[i].x * lineEquation.m) - lineEquation.b + point_vector_list[i].y);
			double dn = sqrt((lineEquation.m * lineEquation.m) + 1);
			double distance = abs(up / dn);
			// ** 여기까지 코드를 작성하세요. 이 줄은 지우시면 안 됩니다 **
			if (distance <= 5)
				inlier.push_back(point_vector_list[i]);
		}
	}
	if (maxInlier < inlier.size())
	{
